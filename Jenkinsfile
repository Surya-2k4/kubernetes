pipeline {
    ...
    environment {
        // Define image names
        BACKEND_IMAGE = "surya2k4/docker-backend:latest"
        FRONTEND_IMAGE = "surya2k4/docker-frontend:latest"
        ...
        stage('Checkout Code') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'demo', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_TOKEN')]) {
                    git url: "https://$GIT_USER:$GIT_TOKEN@github.com/Surya-2k4/kubernetes.git", branch: 'main'
                }
            }
        }
